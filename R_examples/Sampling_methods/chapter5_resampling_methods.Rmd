...
We explore the use of the validation set approach (dev set approach) in order to estimate the test error rates that result from fitting various linear models on the `Auto` data set.
...
```{r}
plot(Auto$horsepower, Auto$mpg)
lines(Auto$horsepower[-train], (predict(lm.fit, Auto))[-train], col='blue')
```
...
points(Auto$horsepower[-train], (predict(lm.fit2, Auto))[-train], col='blue', xlab="HP", ylab="MPG")
```
...
```{r}
plot(Auto$horsepower, Auto$mpg, xlab="HP", ylab="MPG")
points(Auto$horsepower[-train], (predict(lm.fit3, Auto))[-train], col='blue', xlab="HP", ylab="MPG")
```
...
The `cv.glm()` function produces a list with several components.  The two numbers in the `delta` vector contain the cross-validation results. The first component in the vector is the raw cross-validation estimate of prediction error. The second component is the adjusted cross-validation estimate (bias-corrected version).
