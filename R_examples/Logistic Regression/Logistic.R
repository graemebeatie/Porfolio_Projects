...
set.seed(42)
#train.index = sample( c(1:dim(case2001)[1]), .6*dim(case2001)[1] , replace = FALSE)
#comma_vec <- paste(train.index, collapse = ", ")
train.index = c(37, 1, 25, 10, 36, 18, 24, 7, 41, 43, 20, 26, 3, 9, 45, 27, 4, 5, 13, 28, 35, 2, 8, 33, 44, 42, 11)

Glm3 <- glm(Status ~ (Age+Sex)^2, data = case2001, family = binomial, subset = train.index )
Glm4 <- glm(Status ~ Age+Sex, data = case2001, family = binomial, subset = train.index )
Glm5  <- glm(Status ~ Age, data = case2001, family = binomial, subset = train.index )

Glm3.probs = predict(Glm3, case2001[-train.index,] , type = "response") 


glm.pred = rep("Died", length(Glm3.probs) )
glm.pred[Glm3.probs > .5] = "Survived"
#Confusion matrix
table(glm.pred, Status[-train.index])


Glm4.probs = predict(Glm4, case2001[-train.index,] , type = "response") 
glm.pred = rep("Died", length(Glm4.probs) )
glm.pred[Glm4.probs > .5] = "Survived"
#Confusion matrix
table(glm.pred, Status[-train.index])

Glm5.probs = predict(Glm5, case2001[-train.index,] , type = "response") 
glm.pred = rep("Died", length(Glm5.probs) )
glm.pred[Glm5.probs > .5] = "Survived"
#Confusion matrix
table(glm.pred, Status[-train.index])
